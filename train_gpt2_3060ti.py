"""
nanoGPT speedrun adapted for RTX 3060 Ti (8GB VRAM).

Changes from the original RTX 4090 script:
- Removed FP8/torchao (not supported on Ampere, requires Ada Lovelace+)
- Reduced sequence length: 32K -> 4K (memory constraint)
- Reduced gradient accumulation batch size: 16 -> 4
- Increased iterations to compensate for smaller batch
- Added periodic validation loss evaluation
- Updated data paths for cached finewebedu10B
- All architecture and optimizer logic preserved from original
"""
import os
import sys
import glob
...
# -----------------------------------------------------------------------------
# 3. Model Architecture (Modded-GPT) - Identical to original
# -----------------------------------------------------------------------------
...
class Hyperparameters:
    # Use cached finewebedu10B data paths
    input_bin: str = 'finewebedu10B/finewebedu_train_*.bin'
    input_val_bin: str = 'finewebedu10B/finewebedu_val_*.bin'
    # RTX 3060 Ti adaptations:
    # - Sequence length: 32K -> 12K (tested: 5.13 GB peak, well within 8GB VRAM)
    # - Batch size (grad accum): 16 -> 8
    # - Tokens per step: 8 * 12288 = 98,304 (vs original 16 * 32K = 524,288)
    # - 7000 iterations: ~688M tokens total (vs ~896M original)
    # - No FP8 (Ampere doesn't support it), BF16 only
    batch_size: int = 8
...
class DistributedDataLoader:
    def __init__(self, filename_pattern, T):
        ...
        print(f"DataLoader: found {len(self.files)} files matching {filename_pattern}")
        self.reset()
...
# Validation loader
raw_val_loader = DistributedDataLoader(args.input_val_bin, args.sequence_length)
val_loader = AsyncDataLoader(raw_val_loader)

model = GPT(config).bfloat16().cuda()

# No FP8 conversion - RTX 3060 Ti is Ampere (compute 8.6), FP8 requires Ada Lovelace (8.9+)
print("Running in BF16 mode (no FP8 - RTX 3060 Ti)")
...
def evaluate_val():
    ...
    for _ in range(args.val_steps):
        ...
        # Use a fixed window for val
        attn_bs = torch.tensor(1792, dtype=torch.int, device='cuda')
        loss = model(vx, vy, attn_bs)
...
for step in range(args.num_iterations + 1):
    ...
    window = min(window, args.sequence_length)  # Cap at sequence length
    ...
    # Validation
    val_loss_str = ""
    if step % args.val_every == 0 or step == args.num_iterations:
